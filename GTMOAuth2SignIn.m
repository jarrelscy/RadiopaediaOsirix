...
- (BOOL)requestRedirectedToRequest:(NSURLRequest *)redirectedRequest {
  ...
  // if (standardURI != nil && [redirectURI isEqual:standardURI]) return NO; WE ARE NOT USING TITLE STRINGS SINCE RADIOPAEDIA SITE DOES NOT USE THIS
  ...
  if (requestHost && requestPath) {
      NSRange range;
      range.location=0;
      range.length = [[requestURL pathComponents] count] -1;
...
- (BOOL)titleChanged:(NSString *)title {
    return NO; // NOT USING TITLE STRINGS
